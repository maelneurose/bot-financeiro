// Importando da biblioteca que instalamos via Docker
...
// === CLIENTE WHATSAPP (MODERNO - SEM GAMBIARRAS) ===
const client = new Client({
    // LocalAuth: Salva a sessão
    authStrategy: new LocalAuth({ 
        clientId: 'sessao-moderna-2026',
    ...
    // SEM webVersionCache (Deixa a biblioteca nova decidir a melhor versão)
});
...
client.on('message_create', async (msg) => {
    ...
    // Lógica para permitir mensagens do próprio usuário, ignorando as do bot
    ...
    if (texto === 'ver dividas') {
        ...
        return msg.reply(d && d.length ? d.map(x => `${x.type === 'owe' ? '🔴 Devo' : '🟢 Me deve'} ${x.amount} (${x.description})`).join('\n') : '✅ Nada pendente.');
    }
