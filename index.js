// Adicionei 'fetchLatestBaileysVersion' na importação
...
async function connectToWhatsApp() {
    // ⚠️ Troquei a pasta de novo para limpar o erro 405
    const { state, saveCreds } = await useMultiFileAuthState('sessao_v6_version_fix');
    
    // ⚠️ O PULO DO GATO: Busca a versão mais recente do WhatsApp na internet
    console.log('🔄 Buscando versão mais recente do WhatsApp...');
    const { version, isLatest } = await fetchLatestBaileysVersion();
    console.log(`✅ Usando versão: ${version.join('.')} (É a mais recente? ${isLatest})`);

    const sock = makeWASocket({
        version, // ⚠️ Força o uso da versão correta
        auth: {
            creds: state.creds,
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if(connection === 'close') {
            ...
            if(shouldReconnect) {
                // Se der 405 de novo, aumentamos o tempo para não ser bloqueado
    ...
    // ... (restante do código de mensagens igual)
