...
async function connectToWhatsApp() {
    // ⚠️ PASTA DE SESSÃO TOTALMENTE NOVA
    const { state, saveCreds } = await useMultiFileAuthState('auth_baileys_debug_final');
    ...
    const sock = makeWASocket({
        ...
        // ⚠️ Liguei os logs para INFO para vermos o erro real
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if(connection === 'close') {
            ...
            const erroReal = lastDisconnect?.error; // Pega o erro completo
            
            console.log(`🚨 CONEXÃO CAIU! Código: ${reason}`);
            console.log('Detalhe do erro:', erroReal); // Mostra o motivo real no terminal

            const shouldReconnect = reason !== DisconnectReason.loggedOut;
            
            if(shouldReconnect) {
                console.log('⏳ Esperando 5 segundos para reconectar...');
                // ⚠️ Freio de mão: Espera 5s antes de tentar de novo
                setTimeout(connectToWhatsApp, 5000);
            } else {
                console.log('❌ Conexão encerrada permanentemente (Logout). Apague a pasta de sessão.');
            }
    ...
    sock.ev.on('messages.upsert', async m => {
        try {
            ...
            if (['ajuda', 'menu', 'oi'].includes(texto)) await sock.sendMessage(jid, { text: `🤖 *Bot Baileys Online*\n• Gastei 10\n• Devo 50` });
            
            // Lógica completa (igual anterior)
